...
function createExperience(
    ...
    uint256 _numRows, 
    uint256 _numColumns,
    uint256 _minPrice,    // Nuevo parámetro para el precio mínimo
    uint256 _maxPrice     // Nuevo parámetro para el precio máximo
    ...
    Experience memory eventTicket = Experience({
        ...
        location: "",
        date: _date,
        dates: new uint256[](0) , // Corregido para inicializar correctamente el array dinámico vacío
        availableHours: new uint256[](0) , // Corregido igualmente
...
function getSeat(uint256 experienceId, uint256 row, uint256 column) public view returns (uint256, uint256, bool, uint256, address) {
    Experience storage requestedExperience = experiences[experienceId];
    Seat memory seat = requestedExperience.seats[row * requestedExperience.seatTypes.length + column];
    return (seat.row, seat.column, seat.occupied, seat.ticketId, seat.owner);
}

function getSeatType(uint256 experienceId, uint256 seatTypeId) public view returns (string memory, uint256, bool, address) {
    Experience storage requestedExperience = experiences[experienceId];
    SeatType memory seatType = requestedExperience.seatTypes[seatTypeId];
    return (seatType.seat_type, seatType.id, seatType.available, seatType.owner);
}
...
function buyTicketERC20(
    ...
    bool isResale = ownerOf(_ticketId) != address(this);
    uint256 price = isResale ? resalePrice : requestedExperience.ticketPrice;
    ...
    uint256 commissionAmount = price * (isResale ? 3 : 5) / 100; // 3% para reventas, 5% para ventas iniciales

    if (isResale) {
        require(msg.sender != ownerOf(_ticketId), "No puedes comprar tu propio ticket.");

        // Calculando el monto para el vendedor después de la comisión
        address seller = ownerOf(_ticketId);
        uint256 sellerAmount = price - commissionAmount;

        // Transferencia del precio neto al vendedor
        token.transferFrom(msg.sender, seller, sellerAmount);
        // Transferencia de la comisión al contrato (o a la cuenta del organizador)
        token.transferFrom(msg.sender, address(this), commissionAmount);
    } else {
        // Compra inicial de ticket, la comisión se resta del precio y se mantiene en el contrato
        token.transferFrom(msg.sender, address(this), price - commissionAmount);
        token.transferFrom(msg.sender, address(this), commissionAmount);
        requestedExperience.totalTicketsSold++;

        // Opcional: Emitir un certificado de autenticidad para el ticket
    ...
    if (!isResale) {
        ...
        require(selectedSeatType.available, "Tipo de asiento no disponible.");
    ...
    // Actualizar ingresos del evento si es necesario
    if ((address(token) == address(etherToken) || address(token) == address(usdtToken))) {
        if (isResale) {
            // En caso de reventa, actualizar un contador específico de ingresos por reventa si es necesario
        } else {
            // Ventas iniciales
            if (address(token) == address(etherToken)) {
                experienceRevenue[experienceId].etherRevenue += price - commissionAmount;
            } else if (address(token) == address(usdtToken)) {
                experienceRevenue[experienceId].usdtRevenue += price - commissionAmount;
            }
        }
    }
}
